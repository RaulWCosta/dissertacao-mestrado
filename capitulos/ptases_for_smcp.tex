...
Let \(\beta_i\) be a partition of \(B_i\) for some \(i \in I\) and let \(M_i\) be a subgraph of \( G[V_i]\). We denote as \(M_i + \beta_i\) the graph obtained from $M_i$ by adding a new edge \(xy\) for each \((x, y)\) vertex pair that is in the same class in \(\beta_i\). Note that \(M_i + \beta_i\) is not necessarily a subgraph of \(G[V_i]\).
...
\begin{lemma}[\cite{Bateni} Lemma 2.1]\label{bateni_2_1}
...
% TODO rever, escrita parece estranha
...
\begin{theorem}[\cite{Demaine2010} Theorem 1.1] \label{demaineResult}
...
\begin{proof}
    ...
    Tn the line~\ref{alg_smcp_ptas:SpannerCall} of the Algorithm, we construct a spanner \(H\) of \(G_{in}\), which respects the properties described in Lemma~\ref{spanner_spanning_property} and Lemma~\ref{spanner_shortness_property}. That is:
    ...
    Note that Lemma~\ref{spanner_spanning_property} implies that there is a feasible solution for the SMCP contained in \(H\).
