...
The algorithm receives as input a graph \(G_{in}\) with bounded treewidth, and a set \(\mathcal{D} = \{T_1, \dots, T_k\}\) of terminal pairs. It begins by defining a polinomial sized subset of all possible solutions that organize the set of unpaired terminals (from the bottom-up in the dynamic programming tree) in valid ways. We provide a proof that this subset contains a solution not greater than a constant times \(\opt\). The algorithm then calculates the smallest solution, via a dynamic programming approach, that is contained on this subset and outputs it as result.
